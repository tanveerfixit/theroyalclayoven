            ...
            {cart.length === 0 ? (
              ...
              <div className="space-y-6">
                ...
                <button
                  ...
                  className="w-full bg-brand-dark text-white hover:bg-brand-accent py-4 text-sm font-mono font-bold uppercase tracking-widest text-center transition-all flex items-center justify-center space-x-2"
                >
      ...
      {cart.length > 0 && !isCheckoutMode && (
        <div className="fixed bottom-6 left-4 right-4 z-40 lg:hidden animate-slide-up">
          <button
            type="button"
            onClick={() => setIsCheckoutMode(true)}
            className="w-full bg-gradient-to-b from-white/95 to-white/80 backdrop-blur-md text-brand-dark px-5 py-4 flex items-center justify-between border border-brand-dark shadow-[0_12px_40px_-6px_rgba(44,38,33,0.15),inset_0_1px_1px_rgba(255,255,255,0.8)] active:scale-98 transition-all duration-300"
          >
            <div className="flex items-center space-x-3">
              <span className="bg-brand-dark text-white font-mono text-xs w-5 h-5 flex items-center justify-center font-bold">
                {cart.reduce((acc, curr) => acc + curr.quantity, 0)}
              </span>
              <span className="font-mono text-xs uppercase font-bold tracking-widest text-brand-dark">FEAST SELECTED</span>
            </div>
            <div className="flex items-center space-x-2">
              <span className="font-mono text-sm font-bold text-brand-dark">&euro;{total.toFixed(2)}</span>
